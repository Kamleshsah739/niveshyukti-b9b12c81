  ...
  return (
    <main className="min-h-screen bg-background px-4 py-8 md:py-12">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-7 grid gap-4 md:grid-cols-3">{reports.map(({ category, title, detail, icon: Icon }) => <article key={title} className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]"><div className="grid h-11 w-11 place-items-center rounded-xl bg-secondary text-brand-blue"><Icon className="h-5 w-5" /></div><p className="mt-5 text-xs font-bold uppercase tracking-wider text-brand-purple">{category}</p><h2 className="mt-2 text-lg font-bold text-foreground">{title}</h2><p className="mt-2 text-sm text-muted-foreground">{detail}</p><a href="/" className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-brand-blue hover:underline">Return to research tools <LockKeyhole className="h-3.5 w-3.5" /></a></article>)}</div>
      </div>
    </main>
  );
}
